...
namespace FHttpRetrySystem
{
    ...
    public:
		// IJusticeHttpRequest interface
		JUSTICEHTTP_API virtual bool ProcessRequest() override;
		JUSTICEHTTP_API virtual void CancelRequest() override;
		virtual FJusticeHttpRequestCompleteDelegate& OnProcessRequestComplete() override { return OnProcessRequestCompleteDelegate; }
		virtual FJusticeHttpRequestProgressDelegate& OnRequestProgress() override { return OnProcessRequestProgressDelegate; }
    ...
    protected:
		...
		void HttpOnRequestProgress(FJusticeHttpRequestPtr InHttpRequest, int32 BytesSent, int32 BytesRcv);
		...
		FJusticeHttpRequestCompleteDelegate OnProcessRequestCompleteDelegate;
		FJusticeHttpRequestProgressDelegate OnProcessRequestProgressDelegate;
...
namespace FHttpRetrySystem
{
    ...
    public:
        // FManager
		JUSTICEHTTP_API FManager(const FRetryLimitCountSetting& InRetryLimitCountDefault, const FRetryTimeoutRelativeSecondsSetting& InRetryTimeoutRelativeSecondsDefault);

		/**
		 * Create a new http request with retries
		 */
		JUSTICEHTTP_API TSharedRef<class FHttpRetrySystem::FRequest> CreateRequest(
			const FRetryLimitCountSetting& InRetryLimitCountOverride = FRetryLimitCountSetting::Unused(),
			const FRetryTimeoutRelativeSecondsSetting& InRetryTimeoutRelativeSecondsOverride = FRetryTimeoutRelativeSecondsSetting::Unused(),
			const FRetryResponseCodes& InRetryResponseCodes = FRetryResponseCodes()
			);
         ...
         */
        JUSTICEHTTP_API bool Update(uint32* FileCount = NULL, uint32* FailingCount = NULL, uint32* FailedCount = NULL, uint32* CompletedCount = NULL);
		JUSTICEHTTP_API void SetRandomFailureRate(float Value) { RandomFailureRate = FRandomFailureRateSetting::Create(Value); }
