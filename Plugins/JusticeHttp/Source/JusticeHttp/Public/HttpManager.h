...
#include "Private/IJusticeHttpThreadedRequest.h"
 ...
 */
class JUSTICEHTTP_API FHttpManager
...
public:
	 ...
	 */
	void AddRequest(const TSharedRef<IJusticeHttpRequest>& Request);
	 ...
	 */
	void RemoveRequest(const TSharedRef<IJusticeHttpRequest>& Request);
	...
	*/
	bool IsValidRequest(const IJusticeHttpRequest* RequestPtr) const;
...
protected:
	...
	TArray<TSharedRef<IJusticeHttpRequest>> Requests;

	/** Keep track of a request that should be deleted later */
	class FRequestPendingDestroy
	{
	public:
		FRequestPendingDestroy(float InTimeLeft, const TSharedPtr<IJusticeHttpRequest>& InHttpRequest)
			: TimeLeft(InTimeLeft)
		...
		float TimeLeft;
		TSharedPtr<IJusticeHttpRequest> HttpRequest;
	};
