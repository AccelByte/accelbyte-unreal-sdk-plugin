...
/** 
  * Adapter class for IJusticeHttpRequest abstract interface
...
class FHttpRequestAdapterBase : public IJusticeHttpRequest
{
public:
    FHttpRequestAdapterBase(const TSharedRef<IJusticeHttpRequest>& InHttpRequest) 
	...
	virtual const FJusticeHttpResponsePtr        GetResponse() const override                                             { return HttpRequest->GetResponse(); }
	virtual float                         GetElapsedTime() override                                                { return HttpRequest->GetElapsedTime(); }
	virtual EJusticeHttpRequestStatus::Type	  GetStatus() override { return HttpRequest->GetStatus(); }
	virtual void Tick(float DeltaSeconds) override { HttpRequest->Tick(DeltaSeconds); }

protected:
    TSharedRef<IJusticeHttpRequest> HttpRequest;
};
