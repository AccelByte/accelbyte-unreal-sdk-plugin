...
,	ElapsedTime(0.0f)
{
	UE_LOG(LogJusticeHttp, Verbose, TEXT("FAppleHttpRequest::FAppleHttpRequest()"));
	Request = [[NSMutableURLRequest alloc] init];
	Request.timeoutInterval = FJusticeHttpModule::Get().GetHttpTimeout();
...
FString FAppleHttpRequest::GetURL()
{
	...
	if (URL != nullptr)
	{
		...
		UE_LOG(LogJusticeHttp, Verbose, TEXT("FAppleHttpRequest::GetURL() - %s"), *ConvertedURL);
		return ConvertedURL;
	}
	else
	{
		UE_LOG(LogJusticeHttp, Verbose, TEXT("FAppleHttpRequest::GetURL() - NULL"));
		return FString();
	}
}
...
{
	SCOPED_AUTORELEASE_POOL;
	UE_LOG(LogJusticeHttp, Verbose, TEXT("FAppleHttpRequest::SetURL() - %s"), *URL);
...
{
	SCOPED_AUTORELEASE_POOL;
	UE_LOG(LogJusticeHttp, Verbose, TEXT("FAppleHttpRequest::GetURLParameter() - %s"), *ParameterName);
...
FString FAppleHttpRequest::GetHeader(const FString& HeaderName)
{
	...
	UE_LOG(LogJusticeHttp, Verbose, TEXT("FAppleHttpRequest::GetHeader() - %s"), *Header);
	return Header;
}
...
{
	SCOPED_AUTORELEASE_POOL;
	UE_LOG(LogJusticeHttp, Verbose, TEXT("FAppleHttpRequest::SetHeader() - %s / %s"), *HeaderName, *HeaderValue );
...
{
	SCOPED_AUTORELEASE_POOL;
	UE_LOG(LogJusticeHttp, Verbose, TEXT("FAppleHttpRequest::GetAllHeaders()"));
	NSDictionary* Headers = Request.allHTTPHeaderFields;
	...
	for (NSString* Key in Headers.allKeys)
	{
		...
		UE_LOG(LogJusticeHttp, Verbose, TEXT("Header= %s, Key= %s"), *ConvertedValue, *ConvertedKey);
...
{
	SCOPED_AUTORELEASE_POOL;
	UE_LOG(LogJusticeHttp, Verbose, TEXT("FAppleHttpRequest::GetContent()"));
...
{
	UE_LOG(LogJusticeHttp, Verbose, TEXT("FAppleHttpRequest::SetContent()"));
...
FString FAppleHttpRequest::GetContentType()
{
	...
	UE_LOG(LogJusticeHttp, Verbose, TEXT("FAppleHttpRequest::GetContentType() - %s"), *ContentType);
	return ContentType;
}
...
int32 FAppleHttpRequest::GetContentLength()
{
	...
	int Len = Body.length;
	UE_LOG(LogJusticeHttp, Verbose, TEXT("FAppleHttpRequest::GetContentLength() - %i"), Len);
	return Len;
}
...
{
	UE_LOG(LogJusticeHttp, Verbose, TEXT("FAppleHttpRequest::SetContentAsString() - %s"), *ContentString);
...
FString FAppleHttpRequest::GetVerb()
{
	...
	UE_LOG(LogJusticeHttp, Verbose, TEXT("FAppleHttpRequest::GetVerb() - %s"), *ConvertedVerb);
...
{
	SCOPED_AUTORELEASE_POOL;
	UE_LOG(LogJusticeHttp, Verbose, TEXT("FAppleHttpRequest::SetVerb() - %s"), *Verb);
...
{
	SCOPED_AUTORELEASE_POOL;
	UE_LOG(LogJusticeHttp, Verbose, TEXT("FAppleHttpRequest::ProcessRequest()"));
	bool bStarted = false;
	...
	{
		UE_LOG(LogJusticeHttp, Warning, TEXT("ProcessRequest failed. Still processing last request."));
	}
	else if(GetURL().Len() == 0)
	{
		UE_LOG(LogJusticeHttp, Warning, TEXT("ProcessRequest failed. No URL was specified."));
	}
	else if( Scheme != TEXT("http") && Scheme != TEXT("https"))
	{
		UE_LOG(LogJusticeHttp, Warning, TEXT("ProcessRequest failed. URL '%s' is not a valid HTTP request. %p"), *GetURL(), this);
	}
	else
	{
...
FJusticeHttpRequestCompleteDelegate& FAppleHttpRequest::OnProcessRequestComplete()
{
	UE_LOG(LogJusticeHttp, Verbose, TEXT("FAppleHttpRequest::OnProcessRequestComplete()"));
	return RequestCompleteDelegate;
}

FJusticeHttpRequestProgressDelegate& FAppleHttpRequest::OnRequestProgress() 
{
	UE_LOG(LogJusticeHttp, VeryVerbose, TEXT("FAppleHttpRequest::OnRequestProgress()"));
...
{
	SCOPED_AUTORELEASE_POOL;
	UE_LOG(LogJusticeHttp, Verbose, TEXT("FAppleHttpRequest::StartRequest()"));
	bool bStarted = false;
	...
	{
		NSString* Tag = FPlatformJusticeHttp::GetDefaultUserAgent().GetNSString();
...
#pragma clang diagnostic pop
	if (Connection != nullptr && Response->ResponseWrapper != nullptr)
	{
		...
		[Connection start];
		UE_LOG(LogJusticeHttp, Verbose, TEXT("[Connection start]"));
	...
	{
		UE_LOG(LogJusticeHttp, Warning, TEXT("ProcessRequest failed. Could not initialize Internet connection."));
		CompletionStatus = EJusticeHttpRequestStatus::Failed_ConnectionError;
	}
...
{
	UE_LOG(LogJusticeHttp, Verbose, TEXT("FAppleHttpRequest::FinishedRequest()"));
	ElapsedTime = (float)(FPlatformTime::Seconds() - StartRequestTime);
	if( Response.IsValid() && Response->IsReady() && !Response->HadError())
	{
		UE_LOG(LogJusticeHttp, Verbose, TEXT("Request succeeded"));
		CompletionStatus = EJusticeHttpRequestStatus::Succeeded;

		OnProcessRequestComplete().ExecuteIfBound(SharedThis(this), Response, true);
	}
	else
	{
		UE_LOG(LogJusticeHttp, Verbose, TEXT("Request failed"));
	...
	if (FJusticeHttpModule::Get().GetHttpManager().IsValidRequest(this))
	{
		FJusticeHttpModule::Get().GetHttpManager().RemoveRequest(SharedThis(this));
	}
}
...
{
	UE_LOG(LogJusticeHttp, Verbose, TEXT("FAppleHttpRequest::CleanupRequest()"));
...
{
	UE_LOG(LogJusticeHttp, Verbose, TEXT("FAppleHttpRequest::CancelRequest()"));
	if(Connection != nullptr)
	{
		[Connection cancel];
	}
...
EJusticeHttpRequestStatus::Type FAppleHttpRequest::GetStatus()
{
	UE_LOG(LogJusticeHttp, Verbose, TEXT("FAppleHttpRequest::GetStatus()"));
	return CompletionStatus;
}


const FJusticeHttpResponsePtr FAppleHttpRequest::GetResponse() const
{
	return Response;
}
...
{
	if( CompletionStatus == EJusticeHttpRequestStatus::Processing || Response->HadError() )
	{
...
{
	UE_LOG(LogJusticeHttp, Verbose, TEXT("-(FHttpResponseAppleWrapper*) init"));
	self = [super init];
	bIsReady = false;
...
{
	UE_LOG(LogJusticeHttp, Verbose, TEXT("didSendBodyData:(NSInteger)bytesWritten totalBytes:Written:(NSInteger)totalBytesWritten totalBytesExpectedToWrite:(NSInteger)totalBytesExpectedToWrite"));
	self.BytesWritten = totalBytesWritten;
	UE_LOG(LogJusticeHttp, Verbose, TEXT("didSendBodyData: totalBytesWritten = %d, totalBytesExpectedToWrite = %d: %p"), totalBytesWritten, totalBytesExpectedToWrite, self);
}

-(void) connection:(NSURLConnection *)connection didReceiveResponse:(NSURLResponse *)response
{
	UE_LOG(LogJusticeHttp, Verbose, TEXT("didReceiveResponse:(NSURLResponse *)response"));
	...
	UE_LOG(LogJusticeHttp, Verbose, TEXT("didReceiveResponse: expectedContentLength = %d. Length = %d: %p"), [response expectedContentLength], Payload.Max(), self);
}
...
-(void) connection:(NSURLConnection *)connection didReceiveData:(NSData *)data
{
	...
	UE_LOG(LogJusticeHttp, Verbose, TEXT("didReceiveData with %d bytes. After Append, Payload Length = %d: %p"), [data length], Payload.Num(), self);
}
...
-(void) connection:(NSURLConnection *)connection didFailWithError:(NSError *)error
{
	self.bIsReady = YES;
	self.bHadError = YES;
	UE_LOG(LogJusticeHttp, Warning, TEXT("didFailWithError. Http request failed - %s %s: %p"), 
	...
	if (UE_LOG_ACTIVE(LogJusticeHttp, Verbose))
	{
...
#undef MAP_TO_RESULTSTRING
			...
			UE_LOG(LogJusticeHttp, Verbose, TEXT("didFailWithError. SSL trust result: %s (%d)"), *TrustResultString, TrustResult);
		}
	}
}
...
{
	UE_LOG(LogJusticeHttp, Verbose, TEXT("connectionDidFinishLoading: %p"), self);
	self.bIsReady = YES;
}
...
FAppleHttpResponse::FAppleHttpResponse(const FAppleHttpRequest& InRequest)
	: Request( InRequest )
{
	UE_LOG(LogJusticeHttp, Verbose, TEXT("FAppleHttpResponse::FAppleHttpResponse()"));
...
{
	UE_LOG(LogJusticeHttp, Verbose, TEXT("FAppleHttpResponse::GetURL()"));
...
{
	SCOPED_AUTORELEASE_POOL;
	UE_LOG(LogJusticeHttp, Verbose, TEXT("FAppleHttpResponse::GetURLParameter()"));
...
{
	SCOPED_AUTORELEASE_POOL;
	UE_LOG(LogJusticeHttp, Verbose, TEXT("FAppleHttpResponse::GetHeader()"));
...
{
	UE_LOG(LogJusticeHttp, Verbose, TEXT("FAppleHttpResponse::GetAllHeaders()"));
...
{
	UE_LOG(LogJusticeHttp, Verbose, TEXT("FAppleHttpResponse::GetContentType()"));
...
{
	UE_LOG(LogJusticeHttp, Verbose, TEXT("FAppleHttpResponse::GetContentLength()"));
...
const TArray<uint8>& FAppleHttpResponse::GetContent()
{
	if( !IsReady() )
	{
		UE_LOG(LogJusticeHttp, Warning, TEXT("Payload is incomplete. Response still processing. %p"), &Request);
	}
	else
	{
		Payload = [ResponseWrapper getPayload];
		UE_LOG(LogJusticeHttp, Verbose, TEXT("FAppleHttpResponse::GetContent() - Num: %i"), [ResponseWrapper getPayload].Num());
	}
...
{
	UE_LOG(LogJusticeHttp, Verbose, TEXT("FAppleHttpResponse::GetContentAsString()"));
...
{
	UE_LOG(LogJusticeHttp, Verbose, TEXT("FAppleHttpResponse::GetResponseCode()"));
...
{
	UE_LOG(LogJusticeHttp, Verbose, TEXT("FAppleHttpResponse::GetResponseObj()"));
...
bool FAppleHttpResponse::IsReady()
{
	...
	if( Ready )
	{
		UE_LOG(LogJusticeHttp, Verbose, TEXT("FAppleHttpResponse::IsReady()"));
	}
...
bool FAppleHttpResponse::HadError()
{
	...
	if( bHadError )
	{
		UE_LOG(LogJusticeHttp, Verbose, TEXT("FAppleHttpResponse::HadError()"));
	}
