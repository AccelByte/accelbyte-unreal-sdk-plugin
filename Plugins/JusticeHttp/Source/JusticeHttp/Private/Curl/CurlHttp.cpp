	...
	,   LeastRecentlyCachedInfoMessageIndex(0)
{
	...
	// set debug functions (FIXME: find a way to do it only if LogJusticeHttp is >= Verbose)
...
size_t FCurlHttpRequest::ReceiveResponseHeaderCallback(void* Ptr, size_t SizeInBlocks, size_t BlockSizeInBytes)
{
	...
	if (Response.IsValid())
	{
		...
		if (HeaderSize > 0 && HeaderSize <= CURL_MAX_HTTP_HEADER)
		{
			...
			UE_LOG(LogJusticeHttp, Verbose, TEXT("%p: Received response header '%s'."), this, *Header);
		...
		{
			UE_LOG(LogJusticeHttp, Warning, TEXT("%p: Could not process response header for request - header size (%d) is invalid."), this, HeaderSize);
		}
	}
	else
	{
		UE_LOG(LogJusticeHttp, Warning, TEXT("%p: Could not download response header for request - response not valid."), this);
	}
...
size_t FCurlHttpRequest::ReceiveResponseBodyCallback(void* Ptr, size_t SizeInBlocks, size_t BlockSizeInBytes)
{
	...
	if (Response.IsValid())
	{
		...
		UE_LOG(LogJusticeHttp, Verbose, TEXT("%p: ReceiveResponseBodyCallback: %d bytes out of %d received. (SizeInBlocks=%d, BlockSizeInBytes=%d, Response->TotalBytesRead=%d, Response->GetContentLength()=%d, SizeToDownload=%d (<-this will get returned from the callback))"),
			this,
	...
	{
		UE_LOG(LogJusticeHttp, Warning, TEXT("%p: Could not download response data for request - response not valid."), this);
	}
...
size_t FCurlHttpRequest::UploadCallback(void* Ptr, size_t SizeInBlocks, size_t BlockSizeInBytes)
{
	...
	UE_LOG(LogJusticeHttp, Verbose, TEXT("%p: UploadCallback: %d bytes out of %d sent. (SizeInBlocks=%d, BlockSizeInBytes=%d, RequestPayload.Num()=%d, BytesSent=%d, SizeToSend=%d, SizeToSendThisTime=%d (<-this will get returned from the callback))"),
		this,
...
size_t FCurlHttpRequest::DebugCallback(CURL * Handle, curl_infotype DebugInfoType, char * DebugInfo, size_t DebugInfoSize)
{
	...
	switch(DebugInfoType)
	{
		case CURLINFO_TEXT:
			{
				...
				UE_LOG(LogJusticeHttp, VeryVerbose, TEXT("%p: '%s'"), this, *DebugText);
		...
		case CURLINFO_HEADER_IN:
			UE_LOG(LogJusticeHttp, VeryVerbose, TEXT("%p: Received header (%d bytes)"), this, DebugInfoSize);
			break;
		...
		case CURLINFO_HEADER_OUT:
			{
				...
				DebugText.ReplaceInline(TEXT("\n"), TEXT(""), ESearchCase::CaseSensitive);
				DebugText.ReplaceInline(TEXT("\r"), TEXT(""), ESearchCase::CaseSensitive);
				UE_LOG(LogJusticeHttp, VeryVerbose, TEXT("%p: Sent header (%d bytes) - %s"), this, DebugInfoSize, *DebugText);
			}
			break;

		case CURLINFO_DATA_IN:
			UE_LOG(LogJusticeHttp, VeryVerbose, TEXT("%p: Received data (%d bytes)"), this, DebugInfoSize);
			break;

		case CURLINFO_DATA_OUT:
			UE_LOG(LogJusticeHttp, VeryVerbose, TEXT("%p: Sent data (%d bytes)"), this, DebugInfoSize);
			break;

		case CURLINFO_SSL_DATA_IN:
			UE_LOG(LogJusticeHttp, VeryVerbose, TEXT("%p: Received SSL data (%d bytes)"), this, DebugInfoSize);
			break;

		case CURLINFO_SSL_DATA_OUT:
			UE_LOG(LogJusticeHttp, VeryVerbose, TEXT("%p: Sent SSL data (%d bytes)"), this, DebugInfoSize);
			break;

		default:
			UE_LOG(LogJusticeHttp, VeryVerbose, TEXT("%p: DebugCallback: Unknown DebugInfoType=%d (DebugInfoSize: %d bytes)"), this, (int32)DebugInfoType, DebugInfoSize);
			break;
	}
...
bool FCurlHttpRequest::SetupRequest()
{
	...
	UE_LOG(LogJusticeHttp, Verbose, TEXT("%p: URL='%s'"), this, *URL);
	UE_LOG(LogJusticeHttp, Verbose, TEXT("%p: Verb='%s'"), this, *Verb);
	UE_LOG(LogJusticeHttp, Verbose, TEXT("%p: Custom headers are %s"), this, Headers.Num() ? TEXT("present") : TEXT("NOT present"));
	UE_LOG(LogJusticeHttp, Verbose, TEXT("%p: Payload size=%d"), this, RequestPayload.Num());
	...
	{
		UE_LOG(LogJusticeHttp, Log, TEXT("Cannot process HTTP request: URL is empty"));
		return false;
	}	
	...
	{
		UE_LOG(LogJusticeHttp, Fatal, TEXT("Unsupported verb '%s', can be perhaps added with CURLOPT_CUSTOMREQUEST"), *Verb);
	...
	{
		SetHeader(TEXT("User-Agent"), FPlatformJusticeHttp::GetDefaultUserAgent());
	}
	...
	for (int32 Idx = 0; Idx < NumAllHeaders; ++Idx)
	{
		...
		{
			UE_LOG(LogJusticeHttp, Verbose, TEXT("%p: Adding header '%s'"), this, *AllHeaders[Idx]);
		}
...
bool FCurlHttpRequest::ProcessRequest()
{
	...
	if (!SetupRequest())
	{
		UE_LOG(LogJusticeHttp, Warning, TEXT("Could not set libcurl options for easy handle, processing HTTP request failed. Increase verbosity for additional information."));
	...
	FJusticeHttpModule::Get().GetHttpManager().AddThreadedRequest(SharedThis(this));
	
	UE_LOG(LogJusticeHttp, Verbose, TEXT("%p: request (easy handle:%p) has been added to threaded queue for processing"), this, EasyHandle );
	return true;
}
...
bool FCurlHttpRequest::StartThreadedRequest()
{
	...
	UE_LOG(LogJusticeHttp, Verbose, TEXT("%p: request (easy handle:%p) has started threaded processing"), this, EasyHandle);
...
bool FCurlHttpRequest::IsThreadedRequestComplete()
{
	...
	if (bCompleted && ElapsedTime >= FJusticeHttpModule::Get().GetHttpDelayTime())
	{
		return true;
	}
...
FJusticeHttpRequestCompleteDelegate& FCurlHttpRequest::OnProcessRequestComplete()
{
	return RequestCompleteDelegate;
}

FJusticeHttpRequestProgressDelegate& FCurlHttpRequest::OnRequestProgress()
{
...
{
	bCanceled = true;
	UE_LOG(LogJusticeHttp, Verbose, TEXT("%p: HTTP request canceled.  URL=%s"), this, *GetURL());
	
	FHttpManager& HttpManager = FJusticeHttpModule::Get().GetHttpManager();
...
EJusticeHttpRequestStatus::Type FCurlHttpRequest::GetStatus()
{
	return CompletionStatus;
}

const FJusticeHttpResponsePtr FCurlHttpRequest::GetResponse() const
{
	return Response;
}
...
void FCurlHttpRequest::CheckProgressDelegate()
	...
	const bool bProcessing = CompletionStatus == EJusticeHttpRequestStatus::Processing;
...
void FCurlHttpRequest::FinishedRequest()
{
		...
		Response->bSucceeded)
	{
			...
			bDebugServerResponse)
		{
			...
			{
				UE_LOG(LogJusticeHttp, Warning, TEXT("%p: request has been successfully processed. URL: %s, HTTP code: %d, content length: %d, actual payload size: %d"),
					this, *GetURL(), Response->HttpCode, Response->ContentLength, Response->Payload.Num());
			}
			else
			{
				UE_LOG(LogJusticeHttp, Verbose, TEXT("%p: request has been successfully processed. URL: %s, HTTP code: %d, content length: %d, actual payload size: %d"),
					this, *GetURL(), Response->HttpCode, Response->ContentLength, Response->Payload.Num());
			}
	...
	else
	{
		if (CurlAddToMultiResult != CURLM_OK)
		{
			UE_LOG(LogJusticeHttp, Warning, TEXT("%p: request failed, libcurl multi error: %d (%s)"), this, (int32)CurlAddToMultiResult, ANSI_TO_TCHAR(curl_multi_strerror(CurlAddToMultiResult)));
		}
		else
		{
		UE_LOG(LogJusticeHttp, Warning, TEXT("%p: request failed, libcurl error: %d (%s)"), this, (int32)CurlCompletionResult, ANSI_TO_TCHAR(curl_easy_strerror(CurlCompletionResult)));
		}
		...
		for (int32 i = 0; i < InfoMessageCache.Num(); ++i)
		{
			...
			{
				UE_LOG(LogJusticeHttp, Warning, TEXT("%p: libcurl info message cache %d (%s)"), this, (LeastRecentlyCachedInfoMessageIndex + i) % InfoMessageCache.Num(), *(InfoMessageCache[(LeastRecentlyCachedInfoMessageIndex + i) % NumberOfInfoMessagesToCache]));
			}
		}
...
FCurlHttpResponse::FCurlHttpResponse(FCurlHttpRequest& InRequest)
	:	Request(InRequest)
	,	TotalBytesRead(0)
	,	HttpCode(EJusticeHttpResponseCodes::Unknown)
	,	ContentLength(0)
	,	bIsReady(0)
	,	bSucceeded(0)
{
