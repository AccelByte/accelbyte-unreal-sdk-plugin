				...
				for (;;)
				{
					...
					if (Message->msg == CURLMSG_DONE)
					{
						...
						if (Request)
						{
							...
							UE_LOG(LogJusticeHttp, Verbose, TEXT("Request %p (easy handle:%p) has completed (code:%d) and has been marked as such"), CurlRequest, CompletedHandle, (int32)Message->data.result);

							HandlesToRequests.Remove(CompletedHandle);
						}
						else
						{
							UE_LOG(LogJusticeHttp, Warning, TEXT("Could not find mapping for completed request (easy handle: %p)"), CompletedHandle);
						}
					}
				}
			}
...
bool FCurlHttpThread::StartThreadedRequest(IHttpThreadedRequest* Request)
{
	...
	{
		UE_LOG(LogJusticeHttp, Warning, TEXT("Failed to add easy handle %p to multi handle with code %d"), EasyHandle, (int)AddResult);
		return false;
	}
