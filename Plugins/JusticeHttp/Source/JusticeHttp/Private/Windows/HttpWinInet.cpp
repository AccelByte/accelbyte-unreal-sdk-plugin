...
if (bIsDebug) \
{ \
	UE_LOG(LogJusticeHttp, Log, Format, ##__VA_ARGS__); \
} \
else \
{ \
	UE_LOG(LogJusticeHttp, Verbosity, Format, ##__VA_ARGS__); \
}
	...
	::DWORD dwStatusInformationLength)
{
	...
	if (Request != NULL)
	{
		if (!FWinInetConnection::Get().GetHttpManager()->IsValidRequest(Request))
		{
			...
			Request = NULL;
			UE_LOG(LogJusticeHttp, Warning, TEXT("InternetStatusCallbackWinInet: on invalid request %p. "), dwContext);
		}
	}
	...
	case INTERNET_STATUS_REQUEST_COMPLETE:
		...
		if (Request != NULL && Request->GetStatus() == EJusticeHttpRequestStatus::Processing &&
...
bool FWinInetConnection::InitConnection()
{
	...
	UE_LOG(LogJusticeHttp, Log, TEXT("Initializing WinInet connection"));
	...
	UE_LOG(LogJusticeHttp, Log, TEXT("Connected State: %s. Flags: (%s)"), 
		bConnected ? TEXT("Good") : TEXT("Bad"), *ConnectionType);

	// max simultaneous connections allowed by wininet
	::DWORD MaxServerConnections = FJusticeHttpModule::Get().GetHttpMaxConnectionsPerServer();
	...
	{
		UE_LOG(LogJusticeHttp, Warning, TEXT("InternetAttemptConnect failed: %s\n"), 
	...
	InternetHandle = InternetOpen(
		*FPlatformJusticeHttp::GetDefaultUserAgent(),
	...
	{
		UE_LOG(LogJusticeHttp, Warning, TEXT("Failed WinHttpOpen: %s"), 
			*InternetTranslateError(GetLastError()));
		return false;
	}
...
{
	UE_LOG(LogJusticeHttp, Log, TEXT("Closing internet connection"));

	{
		...
		if (InternetHandle != NULL)
		{
			...
			{
				UE_LOG(LogJusticeHttp, Warning, TEXT("InternetCloseHandle failed on the FHttpRequestWinInet: %s"), 
					*InternetTranslateError(GetLastError()));
				return false;
			}
			InternetHandle = NULL;
...
FHttpRequestWinInet::~FHttpRequestWinInet()
{
	UE_LOG(LogJusticeHttp, Verbose, TEXT("Destroying FHttpRequestWinInet %p %p"), this, RequestHandle);
...
bool FHttpRequestWinInet::ProcessRequest()
{
	...
	if (!FJusticeHttpModule::Get().IsHttpEnabled())
	{
		UE_LOG(LogJusticeHttp, Verbose, TEXT("Http disabled. Skipping request. url=%s"), *GetURL());
	}
	// Prevent overlapped requests using the same instance
	else if (CompletionStatus == EJusticeHttpRequestStatus::Processing)
	{
	...
	{
		UE_LOG(LogJusticeHttp, Warning, TEXT("ProcessRequest failed. URL '%s' is not a valid HTTP request. %p"), 
...
bool FHttpRequestWinInet::StartRequest()
{
	...
	if (UE_LOG_ACTIVE(LogJusticeHttp, Verbose))
	{
		for (TMap<FString, FString>::TConstIterator It(RequestHeaders); It; ++It)
		{
			...
			{
				UE_LOG(LogJusticeHttp, Verbose, TEXT("%p Header %s : %s"), this, *It.Key(), *It.Value());
			}
		}
	}
	...
	{
		UE_LOG(LogJusticeHttp, Warning, TEXT("InternetConnect failed: %s"), *InternetTranslateError(GetLastError()));
		return false;
	}
	...
	if (FJusticeHttpModule::Get().GetHttpConnectionTimeout() >= 0)
	{
		uint32 HttpConnectionTimeout =  FJusticeHttpModule::Get().GetHttpConnectionTimeout() == 0 ? 0xFFFFFFFF : FJusticeHttpModule::Get().GetHttpConnectionTimeout() * 1000;
	...
	if (FJusticeHttpModule::Get().GetHttpReceiveTimeout() >= 0)
	{
		uint32 HttpReceiveTimeout = FJusticeHttpModule::Get().GetHttpReceiveTimeout() * 1000;
	...
	if (FJusticeHttpModule::Get().GetHttpSendTimeout() >= 0)
	{
		uint32 HttpSendTimeout = FJusticeHttpModule::Get().GetHttpSendTimeout() * 1000;
	...
	// Query these options to verify
	{
		...
		UE_LOG(LogJusticeHttp, VeryVerbose, TEXT("INTERNET_OPTION_CONNECT_TIMEOUT: %d"), OptionData);
		InternetQueryOption(ConnectionHandle, INTERNET_OPTION_RECEIVE_TIMEOUT, (::LPVOID)&OptionData, (::LPDWORD)&OptionSize);
		UE_LOG(LogJusticeHttp, VeryVerbose, TEXT("INTERNET_OPTION_RECEIVE_TIMEOUT: %d"), OptionData);
		InternetQueryOption(ConnectionHandle, INTERNET_OPTION_SEND_TIMEOUT, (::LPVOID)&OptionData, (::LPDWORD)&OptionSize);
		UE_LOG(LogJusticeHttp, VeryVerbose, TEXT("INTERNET_OPTION_SEND_TIMEOUT: %d"), OptionData);
	}
    ...
    RequestFlags |= INTERNET_FLAG_NO_AUTO_REDIRECT;
	...
	{
		UE_LOG(LogJusticeHttp, Warning, TEXT("HttpOpenRequest failed: %s"), *InternetTranslateError(GetLastError()));
		return false;
	}
	...
	{
		UE_LOG(LogJusticeHttp, Warning, TEXT("InternetSetOption failed: %s"), *InternetTranslateError(GetLastError()));
	}
	...
	{
		UE_LOG(LogJusticeHttp, Warning, TEXT("HttpSendRequest failed: %s"), *InternetTranslateError(GetLastError()));
		return false;
	}
...
void FHttpRequestWinInet::FinishedRequest()
{
		...
		Response->bResponseSucceeded)
	{
		...
		{
			UE_LOG(LogJusticeHttp, Warning, TEXT("Finished request %p. response=%d %s url=%s elapsed=%.3f DownloadSize=%d"),
			this, Response->GetResponseCode(), *GetVerb(), *GetURL(), ElapsedTime, Response->GetContentLength());
		}
		else
		{
			UE_LOG(LogJusticeHttp, Log, TEXT("Finished request %p. response=%d %s url=%s elapsed=%.3f DownloadSize=%d"),
				this, Response->GetResponseCode(), *GetVerb(), *GetURL(), ElapsedTime, Response->GetContentLength());
		}
			...
			bDebugServerResponse)
		{
			...
			for (TArray<FString>::TConstIterator It(AllHeaders); It; ++It)
			{
				...
				if (!HeaderStr.StartsWith(TEXT("Authorization")) && !HeaderStr.StartsWith(TEXT("Set-Cookie")))
				{
					if (bDebugServerResponse)
					{
						UE_LOG(LogJusticeHttp, Warning, TEXT("%p Response Header %s"), this, *HeaderStr);
					}
					else
					{
						UE_LOG(LogJusticeHttp, Verbose, TEXT("%p Response Header %s"), this, *HeaderStr);
					}
				}
			}
		}
	...
	else
	{
		...
		CompletionStatus = (Response.IsValid() && Response->bRequestSent) ? EJusticeHttpRequestStatus::Failed : EJusticeHttpRequestStatus::Failed_ConnectionError;
...
FJusticeHttpRequestCompleteDelegate& FHttpRequestWinInet::OnProcessRequestComplete()
{
	return RequestCompleteDelegate;
}

FJusticeHttpRequestProgressDelegate& FHttpRequestWinInet::OnRequestProgress()
{
...
{
	UE_LOG(LogJusticeHttp, Log, TEXT("Canceling Http request. %p url=%s"),
			this, *GetURL());
...
EJusticeHttpRequestStatus::Type FHttpRequestWinInet::GetStatus()
{
	return CompletionStatus;
}

const FJusticeHttpResponsePtr FHttpRequestWinInet::GetResponse() const
{
	return Response;
}
...
void FHttpRequestWinInet::Tick(float DeltaSeconds)
{
	...
	if (HttpTimeout > 0 &&
		...
		!bDebugVerbose)
	{
		UE_LOG(LogJusticeHttp, Warning, TEXT("Http request taking too long! Elapsed %.3f. Enabling verbose logs %p url=%s"),
	...
	{
		UE_LOG(LogJusticeHttp, Warning, TEXT("Timeout processing Http request. %p url=%s"),
			this, *GetURL());
	...
	// No longer waiting for a response and done processing it
		...
		Response.IsValid() &&
		Response->bIsReady &&
		TotalElapsed >= FJusticeHttpModule::Get().GetHttpDelayTime())
	{
		FinishedRequest();
	}
}
...
,	AsyncBytesRead(0)
,	TotalBytesRead(0)
,	ResponseCode(EJusticeHttpResponseCodes::Unknown)
,	ContentLength(0)
,	bIsReady(0)
,	bResponseSucceeded(0)
,	bRequestSent(0)
,	MaxReadBufferSize(FJusticeHttpModule::Get().GetMaxReadBufferSize())
{
...
FString FHttpResponseWinInet::GetHeader(const FString& HeaderName)
{
	...
	if (!bIsReady)
	{
		UE_LOG(LogJusticeHttp, Warning, TEXT("Can't get cached header [%s]. Response still processing. %p"),
			*HeaderName, &Request);
	}
	else
	{
...
TArray<FString> FHttpResponseWinInet::GetAllHeaders()	
{
	...
	if (!bIsReady)
	{
		UE_LOG(LogJusticeHttp, Warning, TEXT("Can't get cached headers. Response still processing. %p"),&Request);
	}
	else
	{
...
void FHttpResponseWinInet::ProcessResponseHeaders()
{
	...
	if (!HttpQueryInfo(Request.RequestHandle, HTTP_QUERY_RAW_HEADERS_CRLF, NULL, &HeaderSize, NULL))
	{
		...
		{
			UE_LOG(LogJusticeHttp, Warning, TEXT("HttpQueryInfo to get header length for all headers failed: %s. %p"), 
				*InternetTranslateError(GetLastError()), &Request);
		}
		if (HeaderSize == 0)
		{
			UE_LOG(LogJusticeHttp, Warning, TEXT("HttpQueryInfo for all headers returned zero header size. %p"), this);
		}
		...
		{
			UE_LOG(LogJusticeHttp, Warning, TEXT("HttpQueryInfo for all headers failed: %s. %p"), 
				*InternetTranslateError(GetLastError()), &Request);
		}
	...
	{
		UE_LOG(LogJusticeHttp, Warning, TEXT("HttpQueryInfo for all headers failed when trying to determine the size for the header buffer. %p"), 
			&Request);
	}
}
...
void FHttpResponseWinInet::ProcessResponseCode()
{
	...
	{
		UE_LOG(LogJusticeHttp, Warning, TEXT("HttpQueryInfo for response code failed: %s. %p"), 
			*InternetTranslateError(GetLastError()), &Request);
	}
}
	...
	if (!HttpQueryInfo(Request.RequestHandle, HttpQueryInfoLevel, const_cast<TCHAR*>(*HeaderName), &HeaderSize, NULL))
	{
		...
		else if (ErrorCode == ERROR_INSUFFICIENT_BUFFER)
		{
			...
			if (HeaderSizeChars > ARRAYSIZE(HeaderValue) || HeaderNameChars > ARRAYSIZE(HeaderValue))
			{
				...
				UE_LOG(LogJusticeHttp, Verbose, TEXT("Having to resize default buffer for retrieving header %s. Name length: %u. Value length: %u. %p"), 
			...
			{
				UE_LOG(LogJusticeHttp, Warning, TEXT("HttpQueryInfo failed trying to get Header Value for Name %s: %s. %p"), 
...
void FURLWinInet::CrackUrlParameters() const
{
	...
	{
		UE_LOG(LogJusticeHttp, Warning, TEXT("Failed to crack URL parameters for URL:%s"), *RequestURL);
		return;
	}
	...
	if (URLParts.dwExtraInfoLength > 1)
	{
		...
		{
			UE_LOG(LogJusticeHttp, Warning, TEXT("URL '%s' extra info did not start with a '?', so can't parse headers."), *RequestURL);
		}
	}
