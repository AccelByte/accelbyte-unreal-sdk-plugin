...
FHttpRetrySystem::FRequest::FRequest(
	FManager& InManager,
	const TSharedRef<IJusticeHttpRequest>& HttpRequest, 
...
void FHttpRetrySystem::FRequest::HttpOnRequestProgress(FJusticeHttpRequestPtr InHttpRequest, int32 BytesSent, int32 BytesRcv)
{
	...
	const FRetryResponseCodes& InRetryResponseCodes)
{
	return MakeShareable(new FRequest(
		*this,
		FJusticeHttpModule::Get().CreateRequest(),
...
bool FHttpRetrySystem::FManager::ShouldRetry(const FHttpRetryRequestEntry& HttpRetryRequestEntry)
{
	...
	FJusticeHttpResponsePtr Response = HttpRetryRequestEntry.Request->GetResponse();
	...
	if (!Response.IsValid())
	{
		...
		EJusticeHttpRequestStatus::Type Status = HttpRetryRequestEntry.Request->GetStatus();
		if (Status == EJusticeHttpRequestStatus::Failed_ConnectionError)
		{
			bResult = true;
		}
		else if (Status == EJusticeHttpRequestStatus::Failed)
		{
...
bool FHttpRetrySystem::FManager::Update(uint32* FileCount, uint32* FailingCount, uint32* FailedCount, uint32* CompletedCount)
{
	...
	while (index < RequestList.Num())
	{
		...
		EJusticeHttpRequestStatus::Type RequestStatus = HttpRetryRequest->GetStatus();

		if (HttpRetryRequestEntry.bShouldCancel)
		{
			UE_LOG(LogJusticeHttp, Warning, TEXT("Request cancelled on %s"), *(HttpRetryRequest->GetURL()));
		...
		else
		{
			if (!HasTimedOut(HttpRetryRequestEntry, NowAbsoluteSeconds))
			{
				...
				{
					if (RequestStatus != EJusticeHttpRequestStatus::NotStarted)
					{
				...
				if (HttpRetryRequest->Status == FHttpRetrySystem::FRequest::EStatus::Processing)
				{
					...
					if (RequestStatus == EJusticeHttpRequestStatus::Succeeded && RandomFailureRate.bUseValue)
					{
					...
					if (RequestStatus == EJusticeHttpRequestStatus::Failed || RequestStatus == EJusticeHttpRequestStatus::Failed_ConnectionError || RequestStatus == EJusticeHttpRequestStatus::Succeeded)
					{
						bShouldRetry = ShouldRetry(HttpRetryRequestEntry);
						bCanRetry = CanRetry(HttpRetryRequestEntry);
					}

					if (RequestStatus == EJusticeHttpRequestStatus::Failed || RequestStatus == EJusticeHttpRequestStatus::Failed_ConnectionError || forceFail || (bShouldRetry && bCanRetry))
					{
						bIsGreen = false;
						...
						if (forceFail || (bShouldRetry && bCanRetry))
						{
							...
							{
								UE_LOG(LogJusticeHttp, Warning, TEXT("Lockout of %fs on %s"), lockoutPeriod, *(HttpRetryRequest->GetURL()));
							}
						...
						{
							UE_LOG(LogJusticeHttp, Warning, TEXT("Retry exhausted on %s"), *(HttpRetryRequest->GetURL()));
					...
					else if (RequestStatus == EJusticeHttpRequestStatus::Succeeded)
					{
						if (HttpRetryRequestEntry.CurrentRetryCount > 0)
						{
							UE_LOG(LogJusticeHttp, Warning, TEXT("Success on %s"), *(HttpRetryRequest->GetURL()));
						}
				...
				if (HttpRetryRequest->Status == FHttpRetrySystem::FRequest::EStatus::ProcessingLockout)
				{
					if (NowAbsoluteSeconds >= HttpRetryRequestEntry.LockoutEndTimeAbsoluteSeconds)
					{
						...
						if (success)
						{
							UE_LOG(LogJusticeHttp, Warning, TEXT("Retry %d on %s"), HttpRetryRequestEntry.CurrentRetryCount + 1, *(HttpRetryRequest->GetURL()));
			...
			{
				UE_LOG(LogJusticeHttp, Warning, TEXT("Timeout on retry %d: %s"), HttpRetryRequestEntry.CurrentRetryCount + 1, *(HttpRetryRequest->GetURL()));
				bIsGreen = false;
