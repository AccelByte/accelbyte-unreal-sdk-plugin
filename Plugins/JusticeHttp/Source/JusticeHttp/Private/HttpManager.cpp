...
void FHttpManager::Flush(bool bShutdown)
{
	...
	if (bShutdown)
	{
		if (Requests.Num())
		{
			//UE_LOG(LogJusticeHttp, Display, TEXT("Http module shutting down, but needs to wait on %d outstanding Http requests:"), Requests.Num());
		}
		// Clear delegates since they may point to deleted instances
		for (TArray<TSharedRef<IJusticeHttpRequest>>::TIterator It(Requests); It; ++It)
		{
			TSharedRef<IJusticeHttpRequest> Request = *It;
			Request->OnProcessRequestComplete().Unbind();
			Request->OnRequestProgress().Unbind();
			//UE_LOG(LogJusticeHttp, Display, TEXT("	verb=[%s] url=[%s] status=%s"), *Request->GetVerb(), *Request->GetURL(), EJusticeHttpRequestStatus::ToString(Request->GetStatus()));
		}
	}
	...
	while (Requests.Num() > 0)
	{
		...
		if (Requests.Num() > 0)
		{
			...
			{
				UE_LOG(LogJusticeHttp, Display, TEXT("Sleeping 0.5s to wait for %d outstanding Http requests."), Requests.Num());
...
void FHttpManager::AddRequest(const TSharedRef<IJusticeHttpRequest>& Request)
{
	FScopeLock ScopeLock(&RequestLock);
...
void FHttpManager::RemoveRequest(const TSharedRef<IJusticeHttpRequest>& Request)
{
	FScopeLock ScopeLock(&RequestLock);
...
bool FHttpManager::IsValidRequest(const IJusticeHttpRequest* RequestPtr) const
{
	FScopeLock ScopeLock(&RequestLock);

	bool bResult = false;
	for (const TSharedRef<IJusticeHttpRequest>& Request : Requests)
	{
...
void FHttpManager::DumpRequests(FOutputDevice& Ar) const
{
	...
	for (const TSharedRef<IJusticeHttpRequest>& Request : Requests)
	{
		Ar.Logf(TEXT("	verb=[%s] url=[%s] status=%s"),
			*Request->GetVerb(), *Request->GetURL(), EJusticeHttpRequestStatus::ToString(Request->GetStatus()));
	}
}
