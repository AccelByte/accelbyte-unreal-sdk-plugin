...
FString ModifiedUploadedContent = TEXT("Integaration UE4 Upload Modified Test");
...
bool UGCSetup::RunTest(const FString& Parameters)
{
	...
	FlushHttpRequests();
	Waiting(bUGCLoginUserSuccess, "Waiting to logon...");
	...
	FlushHttpRequests();
	Waiting(bUGCDeleteUserSuccess, "Waiting to delete user...");
	...
	bool bUGCAdminLoginSuccess;
	FRegistry::ServerOauth2.LoginWithClientCredentials(FVoidHandler::CreateLambda([&]()
	{
		UE_LOG(LogAccelByteUGCTest, Log, TEXT("Admin Login Success"));
		bUGCAdminLoginSuccess = true;
	}), UGCOnError);
	FlushHttpRequests();
	Waiting(bUGCAdminLoginSuccess, "Waiting admin to logon...");

	AB_TEST_TRUE(bUGCAdminLoginSuccess);

	// Setup UGC configuration
	UGCCreatedChannel = {};
	UGCCreatedType = {};
	UGCCreatedTags.Empty();

	// Check if everything already exist because of failure test.
	...
	FlushHttpRequests();
	Waiting(bGetTypesSuccess, "Waiting to getting types...");
	for(auto type : GetTypesResponse.Data)
	{
		if(type.Type.Equals(UGCType))
		{
			UGCCreatedType = type;
			break;
		}
	...
	FlushHttpRequests();
	Waiting(bGetTagsSuccess, "Waiting to getting tags...");
	for(auto expectedTag : UGCTags)
	{
		for(auto tag : GetTagsResponse.Data)
		{
			if(tag.Tag.Equals(expectedTag))
			{
				UGCCreatedTags.Add(tag);
				break;
			}
		}
	}

	if(UGCCreatedType.Id.IsEmpty())
	{
		bool bCreateTypeSuccess;
		UGCCreateType(UGCType, UGCSubType, THandler<FAccelByteModelsUGCTypeResponse>::CreateLambda([&](const FAccelByteModelsUGCTypeResponse& Response)
		{
			UE_LOG(LogAccelByteUGCTest, Log, TEXT("Success create Type"));
			UGCCreatedType = Response;
			bCreateTypeSuccess = true;
		}), UGCOnError);
		Waiting(bCreateTypeSuccess, "Waiting to create type ...");
		AB_TEST_TRUE(bCreateTypeSuccess);
	}
	
	for(const FString& Tag : UGCTags)
	{
		bool bAlreadyCreated = false;
		for(auto CTag : UGCCreatedTags)
		{
		...
		UGCCreateTags(Tag, THandler<FAccelByteModelsUGCTagResponse>::CreateLambda([&](const FAccelByteModelsUGCTagResponse& Response)
		{
			UE_LOG(LogAccelByteUGCTest, Log, TEXT("Success create tag"));
			UGCCreatedTags.Add(Response);
			bCreateTagSuccess = true;
		}), UGCOnError);
		Waiting(bCreateTagSuccess, "Waiting to create tag ...");
...
{
	bool bAdminLoginSuccess;
	FRegistry::ServerOauth2.LoginWithClientCredentials(FVoidHandler::CreateLambda([&]()
	{
		UE_LOG(LogAccelByteUGCTest, Log, TEXT("Admin Login Success"));
		bAdminLoginSuccess = true;
	}), UGCOnError);
	Waiting(bAdminLoginSuccess, "Waiting admin to logon...");
	
	bool bDeleteTypeSuccess;
	UGCDeleteType(UGCCreatedType.Id, FVoidHandler::CreateLambda([&]()
	{
		UE_LOG(LogAccelByteUGCTest, Log, TEXT("Success Delete Type"));
		UGCCreatedType = {};
		bDeleteTypeSuccess = true;
	}), UGCOnError);
	Waiting(bDeleteTypeSuccess, "Waiting to delete type ...");
	AB_TEST_TRUE(bDeleteTypeSuccess);
	
	for(auto Tag : UGCCreatedTags)
	{
		bool bDeleteTagSuccess = false;
		UGCDeleteTags(Tag.Id, FVoidHandler::CreateLambda([&]()
		{
			UE_LOG(LogAccelByteUGCTest, Log, TEXT("Success Delete Tag"));
			bDeleteTagSuccess = true;
		}), UGCOnError);
		Waiting(bDeleteTagSuccess, "Waiting to delete tag ...");
