...
#include "NboSerializerJustice.h"



FOnlineSessionInfoJustice::FOnlineSessionInfoJustice() :
	HostAddr(NULL),
	SessionId(TEXT("INVALID"))
{
}

void FOnlineSessionInfoJustice::Init(const FOnlineSubsystemJustice& Subsystem)
{
...
/**
 *	Async task for ending a Justice online session
 */
class FOnlineAsyncTaskJusticeEndSession : public FOnlineAsyncTaskBasic<FOnlineSubsystemJustice>
{
private:
...
public:
	FOnlineAsyncTaskJusticeEndSession(class FOnlineSubsystemJustice* InSubsystem, FName InSessionName) :
		FOnlineAsyncTaskBasic(InSubsystem),
		SessionName(InSessionName)
	{
	}

	~FOnlineAsyncTaskJusticeEndSession()
	{
	}
	...
	{
		return FString::Printf(TEXT("FOnlineAsyncTaskJusticeEndSession bWasSuccessful: %d SessionName: %s"), bWasSuccessful, *SessionName.ToString());
	}
...
/**
 *	Async task for destroying a Justice online session
 */
class FOnlineAsyncTaskJusticeDestroySession : public FOnlineAsyncTaskBasic<FOnlineSubsystemJustice>
{
private:
	/** Name of session ending */
	FName SessionName;
...
public:
	FOnlineAsyncTaskJusticeDestroySession(class FOnlineSubsystemJustice* InSubsystem, FName InSessionName) :
		FOnlineAsyncTaskBasic(InSubsystem),
		SessionName(InSessionName)
	{
	}

	~FOnlineAsyncTaskJusticeDestroySession()
	{
	}
	...
	virtual FString ToString() const override
	{
		return FString::Printf(TEXT("FOnlineAsyncTaskJusticeDestroySession bWasSuccessful: %d SessionName: %s"), bWasSuccessful, *SessionName.ToString());
	}
...
bool FOnlineSessionJustice::CreateSession(int32 HostingPlayerNum, FName SessionName, const FOnlineSessionSettings& NewSessionSettings)
{
	...
	if (Session == NULL)
	{
		...
		IOnlineIdentityPtr Identity = JusticeSubsystem->GetIdentityInterface();
		if (Identity.IsValid())
		{
		...
		if (!Session->OwningUserId.IsValid())
		{
			...
			Session->OwningUserName = FString(TEXT("JusticeUser"));
		}
		...
		FOnlineSessionInfoJustice* NewSessionInfo = new FOnlineSessionInfoJustice();
		NewSessionInfo->Init(*JusticeSubsystem);
...
bool FOnlineSessionJustice::CreateSession(const FUniqueNetId& HostingPlayerId, FName SessionName, const FOnlineSessionSettings& NewSessionSettings)
{
	// todo: use proper	HostingPlayerId
	return CreateSession(0, SessionName, NewSessionSettings);
}

bool FOnlineSessionJustice::NeedsToAdvertise()
{
...
bool FOnlineSessionJustice::NeedsToAdvertise( FNamedOnlineSession& Session )
{
	// In Justice, we have to imitate missing online service functionality, so we advertise:
...
bool FOnlineSessionJustice::IsSessionJoinable(const FNamedOnlineSession& Session) const
{
...
uint32 FOnlineSessionJustice::UpdateLANStatus()
{
	...
	if ( NeedsToAdvertise() )
	{
		...
		{
			FOnValidQueryPacketDelegate QueryPacketDelegate = FOnValidQueryPacketDelegate::CreateRaw(this, &FOnlineSessionJustice::OnValidQueryPacketReceived);
...
bool FOnlineSessionJustice::StartSession(FName SessionName)
{
	uint32 Result = E_FAIL;
...
bool FOnlineSessionJustice::UpdateSession(FName SessionName, FOnlineSessionSettings& UpdatedSessionSettings, bool bShouldRefreshOnlineData)
{
...
bool FOnlineSessionJustice::EndSession(FName SessionName)
{
	uint32 Result = E_FAIL;
...
bool FOnlineSessionJustice::DestroySession(FName SessionName, const FOnDestroySessionCompleteDelegate& CompletionDelegate)
{
	uint32 Result = E_FAIL;
...
bool FOnlineSessionJustice::IsPlayerInSession(FName SessionName, const FUniqueNetId& UniqueId)
{
	return IsPlayerInSessionImpl(this, SessionName, UniqueId);
}

bool FOnlineSessionJustice::StartMatchmaking(const TArray< TSharedRef<const FUniqueNetId> >& LocalPlayers, FName SessionName, const FOnlineSessionSettings& NewSessionSettings, TSharedRef<FOnlineSessionSearch>& SearchSettings)
{
...
bool FOnlineSessionJustice::CancelMatchmaking(int32 SearchingPlayerNum, FName SessionName)
{
...
bool FOnlineSessionJustice::CancelMatchmaking(const FUniqueNetId& SearchingPlayerId, FName SessionName)
{
	UE_LOG(LogOnline, Warning, TEXT("CancelMatchmaking is not supported on this platform. Use CancelFindSessions."));
	TriggerOnCancelMatchmakingCompleteDelegates(SessionName, false);
	return false;
...
bool FOnlineSessionJustice::FindSessions(int32 SearchingPlayerNum, const TSharedRef<FOnlineSessionSearch>& SearchSettings)
{
...
bool FOnlineSessionJustice::FindSessions(const FUniqueNetId& SearchingPlayerId, const TSharedRef<FOnlineSessionSearch>& SearchSettings)
{
...
bool FOnlineSessionJustice::FindSessionById(const FUniqueNetId& SearchingUserId, const FUniqueNetId& SessionId, const FUniqueNetId& FriendId, const FOnSingleSessionResultCompleteDelegate& CompletionDelegates)
{
...
uint32 FOnlineSessionJustice::FindLANSession()
{
	...
	FOnValidResponsePacketDelegate ResponseDelegate = FOnValidResponsePacketDelegate::CreateRaw(this, &FOnlineSessionJustice::OnValidResponsePacketReceived);
	FOnSearchingTimeoutDelegate TimeoutDelegate = FOnSearchingTimeoutDelegate::CreateRaw(this, &FOnlineSessionJustice::OnLANSearchTimeout);

	FNboSerializeToBufferJustice Packet(LAN_BEACON_MAX_PACKET_SIZE);
...
bool FOnlineSessionJustice::CancelFindSessions()
{
	uint32 Return = E_FAIL;
...
bool FOnlineSessionJustice::JoinSession(int32 PlayerNum, FName SessionName, const FOnlineSessionSearchResult& DesiredSession)
{
	uint32 Return = E_FAIL;
	FNamedOnlineSession* Session = GetNamedSession(SessionName);
...
bool FOnlineSessionJustice::JoinSession(const FUniqueNetId& PlayerId, FName SessionName, const FOnlineSessionSearchResult& DesiredSession)
{
	// Assuming player 0 should be OK here
	return JoinSession(0, SessionName, DesiredSession);
}

bool FOnlineSessionJustice::FindFriendSession(int32 LocalUserNum, const FUniqueNetId& Friend)
{
	// this function has to exist due to interface definition, but it does not have a meaningful implementation in Justice subsystem
...
bool FOnlineSessionJustice::FindFriendSession(const FUniqueNetId& LocalUserId, const FUniqueNetId& Friend)
{
	// this function has to exist due to interface definition, but it does not have a meaningful implementation in Justice subsystem
	TArray<FOnlineSessionSearchResult> EmptySearchResult;
...
bool FOnlineSessionJustice::FindFriendSession(const FUniqueNetId& LocalUserId, const TArray<TSharedRef<const FUniqueNetId>>& FriendList)
{
	// this function has to exist due to interface definition, but it does not have a meaningful implementation in Justice subsystem
	TArray<FOnlineSessionSearchResult> EmptySearchResult;
	TriggerOnFindFriendSessionCompleteDelegates(0, false, EmptySearchResult);
...
bool FOnlineSessionJustice::SendSessionInviteToFriend(int32 LocalUserNum, FName SessionName, const FUniqueNetId& Friend)
{
	// this function has to exist due to interface definition, but it does not have a meaningful implementation in Justice subsystem
	return false;
};

bool FOnlineSessionJustice::SendSessionInviteToFriend(const FUniqueNetId& LocalUserId, FName SessionName, const FUniqueNetId& Friend)
{
	// this function has to exist due to interface definition, but it does not have a meaningful implementation in Justice subsystem
	return false;
}

bool FOnlineSessionJustice::SendSessionInviteToFriends(int32 LocalUserNum, FName SessionName, const TArray< TSharedRef<const FUniqueNetId> >& Friends)
{
	// this function has to exist due to interface definition, but it does not have a meaningful implementation in Justice subsystem
	return false;
};

bool FOnlineSessionJustice::SendSessionInviteToFriends(const FUniqueNetId& LocalUserId, FName SessionName, const TArray< TSharedRef<const FUniqueNetId> >& Friends)
{
	// this function has to exist due to interface definition, but it does not have a meaningful implementation in Justice subsystem
	return false;
}

uint32 FOnlineSessionJustice::JoinLANSession(int32 PlayerNum, FNamedOnlineSession* Session, const FOnlineSession* SearchSession)
{
	...
	if (Session->SessionInfo.IsValid() && SearchSession != nullptr && SearchSession->SessionInfo.IsValid())
	{
		...
		const FOnlineSessionInfoJustice* SearchSessionInfo = (const FOnlineSessionInfoJustice*)SearchSession->SessionInfo.Get();
		FOnlineSessionInfoJustice* SessionInfo = (FOnlineSessionInfoJustice*)Session->SessionInfo.Get();
...
bool FOnlineSessionJustice::PingSearchResults(const FOnlineSessionSearchResult& SearchResult)
{
	return false;
}

/** Get a resolved connection string from a session info */
static bool GetConnectStringFromSessionInfo(TSharedPtr<FOnlineSessionInfoJustice>& SessionInfo, FString& ConnectInfo, int32 PortOverride=0)
{
	bool bSuccess = false;
...
bool FOnlineSessionJustice::GetResolvedConnectString(FName SessionName, FString& ConnectInfo, FName PortType)
{
	bool bSuccess = false;
	// Find the session
	FNamedOnlineSession* Session = GetNamedSession(SessionName);
	if (Session != NULL)
	{
		TSharedPtr<FOnlineSessionInfoJustice> SessionInfo = StaticCastSharedPtr<FOnlineSessionInfoJustice>(Session->SessionInfo);
...
bool FOnlineSessionJustice::GetResolvedConnectString(const FOnlineSessionSearchResult& SearchResult, FName PortType, FString& ConnectInfo)
{
	bool bSuccess = false;
	if (SearchResult.Session.SessionInfo.IsValid())
	{
		TSharedPtr<FOnlineSessionInfoJustice> SessionInfo = StaticCastSharedPtr<FOnlineSessionInfoJustice>(SearchResult.Session.SessionInfo);
...
FOnlineSessionSettings* FOnlineSessionJustice::GetSessionSettings(FName SessionName) 
{
	FNamedOnlineSession* Session = GetNamedSession(SessionName);
	if (Session)
	{
...
void FOnlineSessionJustice::RegisterLocalPlayers(FNamedOnlineSession* Session)
{
	if (!JusticeSubsystem->IsDedicated())
	{
		IOnlineVoicePtr VoiceInt = JusticeSubsystem->GetVoiceInterface();
		if (VoiceInt.IsValid())
		{
...
void FOnlineSessionJustice::RegisterVoice(const FUniqueNetId& PlayerId)
{
	IOnlineVoicePtr VoiceInt = JusticeSubsystem->GetVoiceInterface();
	if (VoiceInt.IsValid())
	{
		if (!JusticeSubsystem->IsLocalPlayer(PlayerId))
		{
...
void FOnlineSessionJustice::UnregisterVoice(const FUniqueNetId& PlayerId)
{
	IOnlineVoicePtr VoiceInt = JusticeSubsystem->GetVoiceInterface();
	if (VoiceInt.IsValid())
	{
...
bool FOnlineSessionJustice::RegisterPlayer(FName SessionName, const FUniqueNetId& PlayerId, bool bWasInvited)
{
...
bool FOnlineSessionJustice::RegisterPlayers(FName SessionName, const TArray< TSharedRef<const FUniqueNetId> >& Players, bool bWasInvited)
{
	bool bSuccess = false;
	FNamedOnlineSession* Session = GetNamedSession(SessionName);
	if (Session)
...
bool FOnlineSessionJustice::UnregisterPlayer(FName SessionName, const FUniqueNetId& PlayerId)
{
	TArray< TSharedRef<const FUniqueNetId> > Players;
	Players.Add(MakeShareable(new FUniqueNetIdString(PlayerId)));
...
bool FOnlineSessionJustice::UnregisterPlayers(FName SessionName, const TArray< TSharedRef<const FUniqueNetId> >& Players)
{
	bool bSuccess = true;
...
void FOnlineSessionJustice::Tick(float DeltaTime)
{
	SCOPE_CYCLE_COUNTER(STAT_Session_Interface);
	TickLanTasks(DeltaTime);
}

void FOnlineSessionJustice::TickLanTasks(float DeltaTime)
{
	LANSessionManager.Tick(DeltaTime);
}

void FOnlineSessionJustice::AppendSessionToPacket(FNboSerializeToBufferJustice& Packet, FOnlineSession* Session)
{
	...
	SetPortFromNetDriver(*JusticeSubsystem, Session->SessionInfo);

	// Write host info (host addr, session id, and key)
	Packet << *StaticCastSharedPtr<FOnlineSessionInfoJustice>(Session->SessionInfo);

	// Now append per game settings
	AppendSessionSettingsToPacket(Packet, &Session->SessionSettings);
}

void FOnlineSessionJustice::AppendSessionSettingsToPacket(FNboSerializeToBufferJustice& Packet, FOnlineSessionSettings* SessionSettings)
{
#if DEBUG_LAN_BEACON
...
void FOnlineSessionJustice::OnValidQueryPacketReceived(uint8* PacketData, int32 PacketLength, uint64 ClientNonce)
{
...
void FOnlineSessionJustice::ReadSessionFromPacket(FNboSerializeFromBufferJustice& Packet, FOnlineSession* Session)
{
#if DEBUG_LAN_BEACON
	...
	FOnlineSessionInfoJustice* JusticeSessionInfo = new FOnlineSessionInfoJustice();
	JusticeSessionInfo->HostAddr = ISocketSubsystem::Get(PLATFORM_SOCKETSUBSYSTEM)->CreateInternetAddr();
	Packet >> *JusticeSessionInfo;
	Session->SessionInfo = MakeShareable(JusticeSessionInfo); 

	// Read any per object data using the server object
	ReadSettingsFromPacket(Packet, Session->SessionSettings);
}

void FOnlineSessionJustice::ReadSettingsFromPacket(FNboSerializeFromBufferJustice& Packet, FOnlineSessionSettings& SessionSettings)
{
#if DEBUG_LAN_BEACON
...
void FOnlineSessionJustice::OnValidResponsePacketReceived(uint8* PacketData, int32 PacketLength)
{
	...
	if (CurrentSessionSearch.IsValid())
	{
		...
		FNboSerializeFromBufferJustice Packet(PacketData, PacketLength);
...
uint32 FOnlineSessionJustice::FinalizeLANSearch()
{
	if (LANSessionManager.GetBeaconState() == ELanBeaconState::Searching)
	{
		LANSessionManager.StopLANSession();
...
void FOnlineSessionJustice::OnLANSearchTimeout()
{
	FinalizeLANSearch();
...
int32 FOnlineSessionJustice::GetNumSessions()
{
	FScopeLock ScopeLock(&SessionLock);
	return Sessions.Num();
}

void FOnlineSessionJustice::DumpSessionState()
{
	FScopeLock ScopeLock(&SessionLock);
...
void FOnlineSessionJustice::RegisterLocalPlayer(const FUniqueNetId& PlayerId, FName SessionName, const FOnRegisterLocalPlayerCompleteDelegate& Delegate)
{
	Delegate.ExecuteIfBound(PlayerId, EOnJoinSessionCompleteResult::Success);
}

void FOnlineSessionJustice::UnregisterLocalPlayer(const FUniqueNetId& PlayerId, FName SessionName, const FOnUnregisterLocalPlayerCompleteDelegate& Delegate)
{
	Delegate.ExecuteIfBound(PlayerId, true);
}

void FOnlineSessionJustice::SetPortFromNetDriver(const FOnlineSubsystemJustice& Subsystem, const TSharedPtr<FOnlineSessionInfo>& SessionInfo)
{
	auto NetDriverPort = GetPortFromNetDriver(Subsystem.GetInstanceName());
	auto SessionInfoJustice = StaticCastSharedPtr<FOnlineSessionInfoJustice>(SessionInfo);
	if (SessionInfoJustice.IsValid() && SessionInfoJustice->HostAddr.IsValid())
	{
		SessionInfoJustice->HostAddr->SetPort(NetDriverPort);
	}
}

bool FOnlineSessionJustice::IsHost(const FNamedOnlineSession& Session) const
{
	if (JusticeSubsystem->IsDedicated())
	{
		return true;
	...
	IOnlineIdentityPtr IdentityInt = JusticeSubsystem->GetIdentityInterface(); 
