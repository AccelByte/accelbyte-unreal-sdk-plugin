...
#if 0
/**
* This test is disabled since we can not login and doing linked account with twitch	in automation test.
* below test provided as a sample of its implementation 
**/
IMPLEMENT_SIMPLE_AUTOMATION_TEST(FEcommerceTestSyncTwitchDropEntitlement, "AccelByte.Tests.Ecommerce.J.SyncTwichDropEntitlement", AutomationFlagMaskEcommerce);
bool FEcommerceTestSyncTwitchDropEntitlement::RunTest(const FString& Parameters)
{
	// Login using AB Account 
	bool bLoginWithDeviceId = false;
	FRegistry::User.LoginWithDeviceId(FVoidHandler::CreateLambda([&bLoginWithDeviceId]()
	{
		bLoginWithDeviceId = true;
	...
	bool bLikedToTwitch = false;
	// Checking Platform Links 
	FRegistry::User.GetPlatformLinks(THandler<FPagedPlatformLinks>::CreateLambda([&bLikedToTwitch](const FPagedPlatformLinks& links)
	{
		if (links.Data.Num() > 0)
		{
			for (int count = 0; count < links.Data.Num(); count++)
			{
				if (links.Data[count].PlatformId.Equals(TEXT("twitch")))
				{
					bLikedToTwitch = true; 
				}
			}
	...
	// Login With Twitch Platform 
	bool bLoginWithTokenPlatform = false;
	// Token platform is generated by using this link :
	// https://id.twitch.tv/oauth2/authorize?client_id=ey2cccww6qz8gaive15cl31136ukm4&redirect_uri=http://localhost&response_type=%20code&scope=user:edit
	// Twitch account streamed is registered to an organization game account on the twitch 
	// Documentation on confluence : 
	// https://accelbyte.atlassian.net/wiki/spaces/SDETQA/pages/1434779719/How+to+generate+platform+token#TWITCH
	const FString tokenPlatform = TEXT("m30z5283gxl4dwxkax49nzpx6bxdnx"); // Dummy token / should be changed 

	// Make the account linked to twitch if not linked 
	if (!bLikedToTwitch)
	{
		FRegistry::User.LinkOtherPlatform(EAccelBytePlatformType::Twitch, tokenPlatform,
			FVoidHandler::CreateLambda([&bLikedToTwitch]() {
			bLikedToTwitch = true;
		}),
	...
	AB_TEST_TRUE(bLikedToTwitch);

	FRegistry::User.LoginWithOtherPlatform(EAccelBytePlatformType::Twitch, tokenPlatform, FVoidHandler::CreateLambda([&bLoginWithTokenPlatform]()
		{
			bLoginWithTokenPlatform = true;
		}), EcommerceTestErrorHandler);
	WaitUntil(bLoginWithTokenPlatform, "Waiting for login...");
	AB_TEST_TRUE(bLoginWithTokenPlatform);

	bool bSyncTwictDropSuccess = false;
	FAccelByteModelsTwitchDropEntitlement model;
	model.GameId = TEXT("65997"); // Payday 2 on Allen's Twitch Account 
	model.Region = TEXT("US");
	model.Language = TEXT("en-US");
	FRegistry::Entitlement.SyncTwitchDropEntitlement(model,
		FVoidHandler::CreateLambda([&bSyncTwictDropSuccess]()
		{
			bSyncTwictDropSuccess = true;
		}), EcommerceTestErrorHandler);

	WaitUntil(bSyncTwictDropSuccess, "Waiting for query statistic topic rewards...");
	AB_TEST_TRUE(bSyncTwictDropSuccess);
