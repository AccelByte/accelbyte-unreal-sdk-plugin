...
BEGIN_DEFINE_SPEC(FServerStatisticTestSpec, "AccelByte.Tests.ServerStatistic", EAutomationTestFlags::ProductFilter | EAutomationTestFlags::ApplicationContextMask)
	const TArray<FString> StatisticStatCodes = { "sdktest3" , "sdktest4"};
END_DEFINE_SPEC(FServerStatisticTestSpec)

void FServerStatisticTestSpec::Define()
{
	...
	Describe("CreateUserStatItems_Invalid", [this]()
	{
		It("Should fail to create user stat item", [this]()
		{
			...
			WaitUntil(bCreateStatItemDone, "Waiting for stat item created...");
	...
	Describe("CreateUserStatItems_Success", [this]()
	{
		It("Should success to create user stat item", [this]()
		{
			bool bGetUserStatItemDone = false;
			FAccelByteModelsUserStatItemPagingSlicedResult GetUserStatItemResult;
			FRegistry::ServerStatistic.GetUserStatItems(FRegistry::Credentials.GetUserId(), { StatisticStatCodes[0], StatisticStatCodes[1] }, {},
				THandler<FAccelByteModelsUserStatItemPagingSlicedResult>::CreateLambda([&bGetUserStatItemDone, &GetUserStatItemResult](FAccelByteModelsUserStatItemPagingSlicedResult Result)
			{
				GetUserStatItemResult = Result;
				bGetUserStatItemDone = true;
			}), ServerStatisticTestErrorHandler);
			WaitUntil(bGetUserStatItemDone, "Waiting for get stat item...");
			...
			for (int i = 0; i < GetUserStatItemResult.Data.Num(); i++)
			{
				...
				WaitUntil(bDeleteUserStatSuccess, "Waiting for user stat item deletion...");
	...
	{
		It("Should success to get all user stat items", [this]()
		{
	...
	{
		It("Should success to get user stat items by stat codes", [this]()
		{
	...
	{
		It("Should success to get user stat items by tags", [this]()
		{
	...
	Describe("IncrementManyUsersStatItems_Success", [this]()
	{
		It("Should increment many users stat items", [this]()
		{
			bool bCreateStatItemDone = false;
			bool bCreateStatItemSuccess = false;
			TArray<FAccelByteModelsBulkStatItemOperationResult> CreateStatItemResult;
			...
			WaitUntil(bStatItemDeleteSuccess, "Waiting for stat item deleted...");
	...
	Describe("IncrementUserStatItems_Success", [this]()
	{
		It("Should increment user stat items", [this]()
		{
			bool bCreateStatItemDone = false;
			bool bCreateStatItemSuccess = false;
			TArray<FAccelByteModelsBulkStatItemOperationResult> CreateStatItemResult;
