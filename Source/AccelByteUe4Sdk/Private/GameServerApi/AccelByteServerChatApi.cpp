...
#pragma region private utilities
		...
		template <typename T>
		bool ConvertDateTimeFieldsFromUnixTimestamp(const TSet<FString>& InFieldNames, const FJsonObject& InJsonObject, T& OutObject)
		{
...
#pragma endregion 
		...
		ServerChat::ServerChat(const ServerCredentials& Credentials, const ServerSettings& Settings, FHttpRetryScheduler& InHttpRef) : Credentials(Credentials), Settings(Settings), HttpRef(InHttpRef)
		{}

		ServerChat::~ServerChat()
		{}

		void ServerChat::CreateChannelChat(const TSet<FString> MemberUserIds, const TSet<FString> AdminUserIds, const FString& TopicName, const THandler<FAccelByteModelsChatActionCreateTopicServerResponse>& OnSuccess, const FErrorHandler& OnError)
		{
			...
			FString Url = FString::Printf(TEXT("%s/admin/namespaces/%s/topic"), *Settings.ChatServerUrl, *Credentials.GetClientNamespace());
			...
			FRegistry::HttpRetryScheduler.ProcessRequest(Request, CreateHttpResultHandler(OnRequestSuccess, OnError), FPlatformTime::Seconds());
		}

		void ServerChat::DeleteChannelChat(const FString& TopicId, const THandler<FAccelByteModelsChatActionTopicResponse>& OnSuccess, const FErrorHandler& OnError)
		{
			FReport::Log(FString(__FUNCTION__));

			FString Authorization = FString::Printf(TEXT("Bearer %s"), *Credentials.GetClientAccessToken());
			FString Url = FString::Printf(TEXT("%s/admin/namespaces/%s/topic/%s"), *Settings.ChatServerUrl, *Credentials.GetClientNamespace(), *TopicId);
		...
		void ServerChat::AddUserToChannelChat(const FString& TopicId, const FString& UserId, const THandler<FAccelByteModelsChatActionUserTopicResponse>& OnSuccess, const FErrorHandler& OnError)
		{
			FReport::Log(FString(__FUNCTION__));

			FString Authorization = FString::Printf(TEXT("Bearer %s"), *Credentials.GetClientAccessToken());
			FString Url = FString::Printf(TEXT("%s/admin/namespaces/%s/topic/%s/user/%s"), *Settings.ChatServerUrl, *Credentials.GetClientNamespace(), *TopicId, *UserId);
			FString Verb = TEXT("POST");
			FString ContentType = TEXT("application/json");
			FString Accept = TEXT("application/json");
		...
		void ServerChat::RemoveUserFromChannelChat(const FString& TopicId, const FString& UserId, const THandler<FAccelByteModelsChatActionUserTopicResponse>& OnSuccess, const FErrorHandler& OnError)
		{
			FReport::Log(FString(__FUNCTION__));
