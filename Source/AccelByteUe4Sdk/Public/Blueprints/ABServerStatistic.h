...
DECLARE_DYNAMIC_DELEGATE_OneParam(FDGlobalStatItemPagingSlicedDelegate, FAccelByteModelsGlobalStatItemPagingSlicedResult, Response);
...
public:
	...
	UFUNCTION(BlueprintCallable, Category = "AccelByte | Server | Statistic | Api")
	void ListGlobalStatItemsWithFilter(FDGlobalStatItemPagingSlicedDelegate OnSuccess, FDErrorHandler OnError, TArray<FString> const& StatCodes, int32 Limit = 20, int32 Offset = 0);

	UFUNCTION(BlueprintCallable, Category = "AccelByte | Server | Statistic | Api")
	void ListGlobalStatItems(FDGlobalStatItemPagingSlicedDelegate OnSuccess, FDErrorHandler OnError, int32 Limit, int32 Offset);
